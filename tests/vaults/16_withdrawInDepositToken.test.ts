...
describe("Withdraw In Deposit Token", () => {
	...
	// 	try {
	// 		await randomVaults.withdraw_in_deposit_token_mint(
	// 			new BN(1 * 10 ** 6),
	...
	// 	try {
	// 		await vaults.withdraw_in_deposit_token_mint(new BN(1 * 10 ** 6), withdrawer.publicKey, manager);
	// 		expect.fail(
	...
	// 	);
	// 	try {
	// 		await vaults.withdraw_in_deposit_token_mint(
	...
	it("should be able to withdraw", async () => {
		...
		if (initialVtokenBalance.gt(new BN(10 ** -3 * 10 ** 9))) {
			await vaults.withdraw_in_deposit_token(
